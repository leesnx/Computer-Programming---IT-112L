...
Programming Date: 10/05/2022
Activity Number: Midterm Exercise 1
*/
/*Problem:
...
/* Accept the exam score. The exam score must not be greater than the perfect exam score. The do while construct enables the computer to let the user re-enter a score in case the
value entered is invalid. */
        do {
...
// Compute the quiz grade
        quizGrade = (double) quizScore / perfectQuizScore * 50 + 50; // 8 quizGrade = quizScore / perfectQuizScore * 50 + 50
// Compute the exam grade
        examGrade = (double) examScore / perfectExamScore * 50 + 50; // 9 examGrade = examScore / perfectExamScore * 50 + 50
// Compute the subject grade
        prelimGrade = (quizGrade + examGrade) / 2; // 10 prelimGrade = (quizGrade + examGrade)/2
...
// Check if prelim grade is passing then display the grade together with a remark
        if (prelimGrade >= 75) { // 13
            System.out.print("Your Prelim Grade is " + prelimGrade); // 14
            System.out.println(" Remark : PASSED");
        }
// Check if prelim grade is failing then display the grade together with a remark "Failed" // 15
        if (prelimGrade < 75) {
            System.out.print("Your Prelim Grade is " + prelimGrade); // 14
